    ...
    <Box sx={{ padding: 0.5 }}>
      <Grid container spacing={0.5}>
        ...
        <Grid size={{ xs: 12 }}>
          <Grid container spacing={0.5}>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 4, lg: 1.7 }}>
              <Card sx={{ background: '#111b21', borderRadius: 1, boxShadow: '0 1px 2px rgba(0,0,0,0.3)' }}>
                <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 0.2, padding: 0.75 }}>
                  <Typography variant="caption" sx={{ color: '#8696a0', fontSize: '0.65rem' }}>Total Authors</Typography>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mt: 0.1 }}>
                    <Typography variant="h5" sx={{ color: '#e9edef', fontSize: '1.1rem' }}>{stats.counts.authors}</Typography>
                    <Box sx={{ fontSize: 10, px: 0.5, py: 0.15, borderRadius: 999, bgcolor: 'rgba(241,178,51,0.1)', color: '#f1b233' }}>
                      {formatTrend(uploadsTrend)}
                    </Box>
                  </Box>
                  <Typography variant="caption" sx={{ color: '#8696a0', mt: 0.1, fontSize: '0.6rem' }}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Card sx={{ background: '#111b21', borderRadius: 1 }}>
            <CardContent sx={{ padding: 1 }}>
              <Typography variant="subtitle1" sx={{ color: '#e9edef', mb: 0.3, fontSize: '0.85rem' }}>Top Authors by Books</Typography>
              <Divider sx={{ borderColor: '#202c33', mb: 0.5 }} />
              <Box sx={{ overflowX: 'auto', maxHeight: 350, overflowY: 'auto' }}>
                <table className="table overview-table">
                  <thead>
                  ...
                  <tbody>
                    {(stats.authors && stats.authors.length > 0 ? stats.authors : []).slice((authorsPage - 1) * OVERVIEW_PAGE_SIZE, authorsPage * OVERVIEW_PAGE_SIZE).map((a, i) => (
                      <tr key={i}>
                        <td>
                          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                            <span>{a.name === 'Unknown' ? '👤 Unknown' : a.name}</span>
                          </Box>
                        </td>
                        <td><Box sx={{ color: '#00a884', fontWeight: 600 }}>{a.bookCount}</Box></td>
                        <td>{a.totalDownloads}</td>
                        <td>
                          <Box sx={{ color: '#f1b233', fontWeight: 600 }}>
                            {a.avgRating > 0 ? `${a.avgRating}⭐` : '—'}
                          </Box>
                        </td>
                      </tr>
                    ))}
                    {(!stats.authors || stats.authors.length === 0) && (
                      <tr><td colSpan={4} style={{ color: '#8696a0' }}>No author data</td></tr>
                    )}
                  </tbody>
                </table>
              </Box>
              {stats.authors && stats.authors.length > OVERVIEW_PAGE_SIZE && (
                <Box className="actions" sx={{ mt: 1.5, display: 'flex', justifyContent: 'space-between' }}>
                  <button
                    className="btn"
                  ...
                  <span style={{ color: '#cfd8dc', fontSize: 12 }}>
                    Page {authorsPage} of {Math.max(1, Math.ceil((stats.authors.length || 0) / OVERVIEW_PAGE_SIZE))}
                  </span>
                  <button
                    className="btn"
                    disabled={authorsPage >= Math.ceil((stats.authors.length || 0) / OVERVIEW_PAGE_SIZE)}
                    onClick={() => setAuthorsPage(p => Math.min(Math.ceil((stats.authors.length || 0) / OVERVIEW_PAGE_SIZE), p + 1))}
                  >
                    Next
                  </button>
                </Box>
