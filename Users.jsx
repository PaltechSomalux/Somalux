...
import { fetchProfiles, fetchUploadCountsByUser, updateUserRole, fetchUserRankingsAdmin } from '../api';
...
const Users = ({ isSuperAdmin }) => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [profiles, uploadCounts, rankings] = await Promise.all([
        fetchProfiles(),
        fetchUploadCountsByUser(),
        fetchUserRankingsAdmin(),
      ]);
      ...
      console.error('[Users.load] Fatal error:', error);
      setRows([]);
    } finally {
      setLoading(false);
    }
