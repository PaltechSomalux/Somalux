...
import { FiRefreshCw, FiFileText, FiMapPin } from 'react-icons/fi';
...
const Submissions = ({ userProfile }) => {
  ...
  const [type, setType] = useState('past_papers'); // 'past_papers' | 'universities'
  const [summary, setSummary] = useState({ pastPapersPending: 0, universitiesPending: 0 });
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      // Use submissions endpoint for all types (past_papers, universities)
  ...
  const approve = async (id) => {
    ...
    const ok = await confirm({
      title: `Approve ${type === 'universities' ? 'University' : 'Past Paper'}?`,
    ...
    setSummary((prev) => {
      const updated = { ...prev };
        if (type === 'universities') updated.universitiesPending = Math.max(0, updated.universitiesPending - 1);
    ...
    try {
      ...
      if (type === 'universities') {
        ...
        // For past_papers, use submissions endpoint
      ...
      showToast({ type: 'success', message: `${type === 'universities' ? 'University' : 'Past paper'} approved successfully.` });
    } catch (e) {
  ...
  const reject = async (id) => {
    ...
    const reason = await prompt({
      title: `Reject ${type === 'universities' ? 'University' : 'Past Paper'}?`,
    ...
    try {
      ...
      showToast({ type: 'success', message: `${type === 'universities' ? 'University' : 'Past paper'} rejected.` });
    } catch (e) {
  ...
  return (
    <div className="panel">
      ...
      <div style={{ color: '#8696a0', fontSize: 12, marginBottom: 8 }}>
        Review and approve or reject user-contributed universities and past papers.
      </div>
