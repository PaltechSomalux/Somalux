...
const Settings = ({ userProfile }) => {
  // Simple placeholder logo data URL (can be replaced with actual logo)
  const logo = 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"%3E%3Crect fill="%2300a884" width="100" height="100"/%3E%3Ctext x="50" y="60" font-size="40" fill="white" text-anchor="middle"%3EP%3C/text%3E%3C/svg%3E';
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (pdfOptions.viewsDetails && viewDetails.length > 0) {
        ...
        doc.setFontSize(16);
        doc.setFont('helvetica', 'bold');
        doc.text('Views Details', 15, yPos);
        const startY = yPos + 6;
        autoTable(doc, {
          startY,
          head: [['Total Views', 'Unique Users']],
  ...
  return (
    <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 6, marginBottom: 8 }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <input
                ...
                style={{ width: 16, height: 16, cursor: 'pointer' }}
              />
              <span style={{ color: '#e9edef' }}>Views Details</span>
            </label>
          </div>
      ...
      <div className="panel">
        ...
        <input className="input" placeholder="storage" />
