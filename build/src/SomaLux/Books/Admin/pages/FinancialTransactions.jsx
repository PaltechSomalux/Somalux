...
import {
  ...
  Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Alert, Box,
  Select, MenuItem, FormControl, InputLabel, Chip, Tab, Tabs, Card, CardContent,
  Grid, LinearProgress, Switch, FormControlLabel
} from '@mui/material';
import { FiDownload, FiPlus, FiEdit, FiTrash2, FiTrendingUp } from 'react-icons/fi';
  ...
  return (
    <div className="assets-container">
      ...
      <div className="assets-header-simple">
        ...
        <button className="btn" onClick={() => { setEditingId(null); setDialogType('transaction'); setFormData({...formData, description: '', amount: ''}); setOpenDialog(true); }} style={{ padding: '6px 12px', fontSize: '12px', background: '#00a884', borderRadius: '4px', marginLeft: 'auto' }}>+ Add</button>
      </div>
              ...
              <TableBody>
                {getFilteredTransactions().map(t => (
                  <TableRow key={t.id} className="table-row">
                    ...
                    <TableCell align="center"><button className="action-btn" onClick={() => { setEditingId(t.id); setFormData(t); setDialogType('transaction'); setOpenDialog(true); }} style={{marginRight:'4px'}}><FiEdit style={{fontSize:'12px'}} /></button><button className="action-btn" onClick={() => handleDeleteTransaction(t.id)} style={{color:'#e74c3c'}}><FiTrash2 style={{fontSize:'12px'}} /></button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
