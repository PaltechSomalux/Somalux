...
import { ProfileViewer } from '../KissMe/Components/ProfileViewer';
...
import {
  collection, addDoc, serverTimestamp, doc, updateDoc, writeBatch,
  deleteDoc, arrayUnion, runTransaction, getDoc, setDoc, onSnapshot
...
export const ChatScreen = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(groupRef, (docSnap) => {
      if (docSnap.exists()) {
        const updatedGroup = { id: docSnap.id, ...docSnap.data() };
        // Prefer memberIds length; fallback to members length
        const byIds = Array.isArray(updatedGroup.memberIds) ? updatedGroup.memberIds.length : null;
        const byMembers = Array.isArray(updatedGroup.members) ? updatedGroup.members.length : null;
        updatedGroup.memberCount = (byIds ?? byMembers ?? 0);
  ...
  const {
    ...
    initialMessages: [],
    groupMemberCount: group?.memberCount || (Array.isArray(group?.members) ? group.members.length : null),
  });
  ...
  const isAdmin = !!(currentUser && group && (group.createdBy === currentUser.uid || (group.admins || []).includes(currentUser.uid)));
  const adminOnly = !!group?.onlyAdminsCanSend;
  ...
  // Update read marker in groupReads subcollection
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        await setDoc(doc(db, 'groups', gid, 'groupReads', uid), {
          userId: uid,
          groupId: gid,
  ...
  }, [currentUser?.uid, group?.id, messages.length]);
  ...
  const deleteSelectedMessages = async () => {
    ...
    try {
      ...
      selectedMessages.forEach(id => {
        const ref = doc(db, 'groups', group.id, 'messages', id);
        batch.update(ref, { deletedBy: arrayUnion(currentUser.uid) });
      });
      await batch.commit();

      // Fixed line – was using undefined `cmds`
  ...
  const handleOpenProfile = (senderId, senderName) => {
    ...
    const profile = member ? {
      ...
      isCurrent: currentUser && (member.uid === currentUser.uid || member.id === currentUser.uid),
    } : {
      id: senderId,
      name: senderName || 'User',
      profilePicture: null,
      isCurrent: currentUser && (senderId === currentUser.uid),
    };
  ...
  const handleDirectMessage = async (message) => {
    try {
      ...
      if (!me || !message?.sender || message.sender === me.uid) return;

      // Ensure chat entry exists in userChats for current user
      const chatRef = doc(db, 'userChats', me.uid, 'chats', message.sender);
      const snap = await getDoc(chatRef);
      if (!snap.exists()) {
        await setDoc(chatRef, {
          contactUid: message.sender,
    ...
    try {
      for (const targetId of targetGroupIds) {
        for (const msg of messages) {
          await addDoc(collection(db, 'groups', targetId, 'messages'), {
            text: msg.text || '',
  ...
  const saveEditedMessage = async () => {
    ...
    try {
      ...
      const ref = doc(db, 'groups', group.id, 'messages', messageEdit.id);
      await updateDoc(ref, { text, edited: true });
    } catch (e) {
  ...
  const deleteMessageForMe = async (messageId) => {
    ...
    try {
      deleteForMeLocal([messageId], currentUser.uid);
      const ref = doc(db, 'groups', group.id, 'messages', messageId);
      await updateDoc(ref, { deletedBy: arrayUnion(currentUser.uid) });
  ...
  const deleteMessageForEveryone = async (messageId) => {
    ...
    const isAdmin = group.createdBy === currentUser.uid || group.admins?.includes(currentUser.uid);
  ...
  const addReaction = async (messageId, emoji) => {
    ...
    const ref = doc(db, 'groups', group.id, 'messages', messageId);
    try {
      reactToMessage(messageId, currentUser.uid, emoji);
      await runTransaction(db, async (trx) => {
        const snap = await trx.get(ref);
        if (!snap.exists()) return;
        const data = snap.data();
        const reactions = { ...(data.reactions || {}) };
        const prev = reactions[currentUser.uid];
        if (prev === emoji) {
        ...
        trx.update(ref, { reactions, reactionsCounts: counts });
      });
    } catch (err) {
      reactToMessage(messageId, currentUser.uid, emoji);
  ...
  const clearChatForMe = async () => {
    ...
    try {
      const ids = messages
        .filter(m => !(m.deletedBy || []).includes(currentUser.uid))
        .map(m => m.id);
      deleteForMeLocal(ids, currentUser.uid);
      const chunkSize = 50;
      for (let i = 0; i < ids.length; i += chunkSize) {
        const chunk = ids.slice(i, i + chunkSize);
        const existing = [];
        for (const id of chunk) {
          const snap = await getDoc(doc(db, 'groups', group.id, 'messages', id));
          if (snap.exists()) existing.push(id);
        }
        await Promise.all(existing.map(id =>
          updateDoc(doc(db, 'groups', group.id, 'messages', id), {
  ...
  const togglePinMessage = async (messageOrId) => {
    ...
    try {
      ...
      setPinnedMessages(prev =>
        isPinned ? prev.filter(m => m.id !== msg.id) : [...prev, { ...msg, pinned: true }]
      );
    } catch (error) {
    ...
    <div className="imo-chat-screen">
      ...
      {adminOnly && !isAdmin ? (
        ...
        <InputArea
          ...
          currentUser={currentUser ? { id: currentUser.uid, name: currentUser.displayName || currentUser.email?.split('@')[0] } : null}
