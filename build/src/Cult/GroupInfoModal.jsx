...
import { doc, updateDoc, arrayUnion, arrayRemove, getDoc, onSnapshot, addDoc, collection, serverTimestamp } from 'firebase/firestore';
...
export const GroupInfoModal = ({ group: initialGroup, onClose, onGroupUpdate, onOpenProfile }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(groupRef, (docSnap) => {
      if (docSnap.exists()) {
        setLocalGroup({ id: docSnap.id, ...docSnap.data() });
  ...
  const isAdmin = currentUser && (
    localGroup?.createdBy === currentUser.uid || 
    localGroup?.admins?.includes(currentUser.uid)
  );

  const isUserAdmin = (userId) => {
    return userId === localGroup?.createdBy || localGroup?.admins?.includes(userId);
  };

  // Toggle onlyAdminsCanSend
  const handleToggleSendRestriction = async () => {
    if (!isAdmin || !localGroup?.id) return;
    const next = !localGroup?.onlyAdminsCanSend;
    setLoading(true);
    try {
      const groupRef = doc(db, 'groups', localGroup.id);
      await updateDoc(groupRef, { onlyAdminsCanSend: next, lastActivity: serverTimestamp() });
      // System message
      try {
        const actor = currentUser?.displayName || currentUser?.email?.split('@')[0] || 'Admin';
        await addDoc(collection(db, 'groups', localGroup.id, 'messages'), {
          sender: 'system', senderName: 'System', type: 'system',
          text: next ? `Only admins can send messages (set by ${actor})` : `All members can send messages (set by ${actor})`,
          timestamp: serverTimestamp(), status: 'delivered', readBy: [], isPinned: false,
        });
      } catch {}
    } catch (e) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      try {
        const actor = currentUser?.displayName || currentUser?.email?.split('@')[0] || 'Admin';
        const changes = [name ? 'name' : null, icon ? 'icon' : null].filter(Boolean).join(' & ');
        await addDoc(collection(db, 'groups', localGroup.id, 'messages'), {
          sender: 'system', senderName: 'System', type: 'system',
          text: `Group ${changes} updated by ${actor}`,
          timestamp: serverTimestamp(), status: 'delivered', readBy: [], isPinned: false,
        });
      } catch {}
  ...
  const handleMakeAdmins = async () => {
    ...
    try {
      ...
      await updateDoc(groupRef, {
        admins: arrayUnion(...promotableIds)
      });
      
      // Updates will reflect immediately via onSnapshot
  ...
  const handleRemoveAdmins = async () => {
    ...
    const demotableIds = selectedMemberIds.filter(id => 
      isUserAdmin(id) && localGroup.createdBy !== id
    );
    ...
    try {
      const groupRef = doc(db, 'groups', localGroup.id);
      await updateDoc(groupRef, {
        admins: arrayRemove(...demotableIds)
      });
  ...
  const handleRemoveMembers = async () => {
    ...
    try {
      const groupRef = doc(db, 'groups', localGroup.id);
      await updateDoc(groupRef, {
        members: arrayRemove(...removableMembers),
        memberIds: arrayRemove(...removableMembers),
        admins: arrayRemove(...removableMembers) // Also remove from admins if applicable
      });

      // Post a system message about removals
      try {
        const removedNames = members
          .filter(m => removableMembers.includes(m.uid))
          .map(m => m.displayName || m.email || m.uid);
        const removerName = currentUser?.displayName || currentUser?.email?.split('@')[0] || 'Admin';
        const text = removedNames.length === 1
          ? `${removedNames[0]} was removed by ${removerName}`
          : `${removedNames.join(', ')} were removed by ${removerName}`;
        await addDoc(collection(db, 'groups', localGroup.id, 'messages'), {
          sender: 'system',
          senderName: 'System',
          text,
        ...
        console.warn('Failed to write removal system message', e);
      }

      // Updates will reflect immediately via onSnapshot and useEffect
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      ...
      // localGroup will be updated via onSnapshot
      ...
      <div className="group-info-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="group-info-content">
          ...
          <div className="group-info-left">
            {/* Group Image */}
            <div className="group-info-image-section" style={{ position: 'relative' }}>
              {localGroup?.icon ? (
                <img 
                  src={localGroup.icon} 
                  alt={localGroup.name} 
                  ...
                  onError={(e) => {
                    ...
                    if (parent && !parent.querySelector('.group-info-image-placeholder')) {
                      const div = document.createElement('div');
                      div.className = 'group-info-image-placeholder';
              ...
              <h3 className="group-name">{localGroup?.name || 'Group Chat'}</h3>
              <p className="group-members-count">{members.length} members</p>
            </div>
            ...
            <div className="group-info-footer">
              <p className="group-created-info">
