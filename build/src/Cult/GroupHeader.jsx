...
export const GroupHeader = ({
  ...
  const getGroupMemberStatus = () => {
    ...
    if (enableFeatures.typingIndicators && isTyping) {
      ...
      return (
        <div className="groupHeader-status-container">
          <span className="groupHeader-typingIndicator">
            <span className="typing-dot"></span>
            <span className="typing-dot"></span>
            <span className="typing-dot"></span>
            {label}
          </span>
          <span className="groupHeader-member-count">{totalMembersText}</span>
        </div>
      );
    }
    ...
    if (enableFeatures.onlineStatus && !isSelfChat) {
      ...
      if (online > 0) {
        return (
          <div className="groupHeader-status-container">
            <span className="groupHeader-onlineStatus">
              <span className="groupHeader-onlineDot"></span>
      ...
      if (group.lastActivity) {
        return (
          <div className="groupHeader-status-container">
            <span className="groupHeader-last-activity">Last active {formatDate(group.lastActivity)}</span>
            <span className="groupHeader-member-count">{totalMembersText}</span>
          </div>
        );
      }
  ...
  return (
    <div className={`groupHeader-header ${currentTheme}-theme`}>
      {showGroupAvatarFullscreen && (
        <div className="groupHeader-avatarFullscreenOverlay" onClick={() => setShowGroupAvatarFullscreen(false)}>
          <div className="groupHeader-avatarFullscreenContainer" onClick={e => e.stopPropagation()}>
            <img
              src={group.avatar || generateDefaultGroupAvatar()}
              alt={group.name}
              className="groupHeader-avatarFullscreenImage"
              onError={(e) => { e.currentTarget.src = generateDefaultGroupAvatar(); }}
            />
            <div className="groupHeader-avatarFullscreenFooter">
              <h3>{group.name}</h3>
              <div className="groupHeader-avatarActions">
            ...
            <button className="groupHeader-avatarCloseButton" onClick={() => setShowGroupAvatarFullscreen(false)}>
      ...
      <button className="groupHeader-backButton" onClick={onBackClick}>
        <FiArrowLeft size={24} strokeWidth={2.5} style={{ display: 'block', visibility: 'visible', opacity: 1 }} />
      </button>

      <div className="groupHeader-contactInfo" onClick={() => setShowGroupInfo(true)} role="button" tabIndex={0}>
        <div className="groupHeader-contactAvatar">
          {group.avatar && !avatarFailed ? (
            ...
            <div className="groupHeader-avatarPlaceholder">
              {group?.name?.charAt(0)?.toUpperCase() || 'G'}
            </div>
          )}
        </div>
        <div className="groupHeader-contactDetails">
          <h3>{group.name}</h3>
          <p className="groupHeader-status">{getGroupMemberStatus()}</p>
        </div>
      </div>

      {pinnedMessages.length > 0 && (
        <div className="groupHeader-pinned-container">
          <div className="groupHeader-pinned-header">
            <BsPinFill size={12} className="groupHeader-pinned-icon" />
            <span className="groupHeader-pinned-label">Pinned</span>
          </div>
          <div className="groupHeader-pinned-list" ref={pinnedRef}>
            {pinnedMessages.map(pin => (
              <div key={pin.id} className="groupHeader-pinned-item" role="button" tabIndex={0}>
                <div className="groupHeader-pinned-content" onClick={() => onPinClick(pin, true)}>
                  <div className="groupHeader-pinned-text">
                    {pin.text?.substring(0, 30)}{pin.text?.length > 30 ? '...' : ''}
                  </div>
                  <div className="groupHeader-pinned-time">{formatDate(pin.timestamp)}</div>
                </div>
                <button className="groupHeader-pinned-unpin" onClick={(e) => { e.stopPropagation(); onUnpinMessage(pin); }}>
                  <FiX size={14} />
                </button>
              </div>
            ))}
      ...
      <div className="groupHeader-chatMenuContainer" ref={menuRef}>
        <button className="groupHeader-iconButton groupHeader-voiceButton" onClick={onVoiceCallClick}>
          <FiPhone size={24} strokeWidth={2.5} />
        </button>
        <button className="groupHeader-iconButton groupHeader-videoButton" onClick={onVideoCallClick}>
          <FiVideo size={24} strokeWidth={2.5} />
        </button>
        <button className="groupHeader-iconButton groupHeader-menuButton" onClick={() => setShowChatMenu(!showChatMenu)}>
