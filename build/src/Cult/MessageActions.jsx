...
export const MessageActions = ({
  ...
  const modal = (
    <>
      ...
      <div className="group-message-actions-container">
        <div className="group-message-actions-backdrop" onClick={onClose} />
        <div className="group-message-actions-modal">
          {showCopiedNotification ? (
            ...
            <div className="delete-options-group">
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
                {['❤️','👍','😂','😲','😢','🙏'].map(e => (
                  <button key={e} className="group-action-button" onClick={() => handleReact(e)}>
            ...
            <div className="delete-options-group">
              ...
              {canDeleteForEveryone && (
                <button
                  className="group-action-button delete-button-group"
                  onClick={() => handleDelete(true)}
                >
                  <Icon name="trash" className="group-action-icon" />
            ...
            <div className="group-message-actions-grid">
              {canReply && (
                <button className="group-action-button" onClick={handleReply}>
                  <Icon name="reply" className="group-action-icon" />
                  <span>Reply</span>
                </button>
              )}

              {canDM && (
                <button className="group-action-button" onClick={handleDirectMessage}>
                  <Icon name="message" className="group-action-icon" />
                  <span>DM</span>
                </button>
              )}

              {canForward && (
                <button className="group-action-button" onClick={handleForward}>
                  <Icon name="share" className="group-action-icon" />
                  <span>Forward</span>
                </button>
              )}

              {canPin && (
                <button className="group-action-button" onClick={handlePin}>
                  <Icon name="bookmark" className="group-action-icon" />
              ...
              {canEdit && (
                <button className="group-action-button" onClick={handleEdit}>
                  <Icon name="edit" className="group-action-icon" />
                  <span>Edit</span>
                </button>
              )}

              {canCopy && (
                <button className="group-action-button" onClick={handleCopy}>
                  <Icon name="copy" className="group-action-icon" />
                  <span>Copy</span>
                </button>
              )}
              ...
              {canStar && (
                <button 
                  className={`group-action-button ${message.isStarred ? 'starred-group' : ''}`}
                  onClick={handleStar}
                >
                  <Icon name="star" className={`group-action-icon ${message.isStarred ? 'starred-icon-group' : ''}`} />
                  <span>React</span>
                </button>
              )}

              {canViewInfo && (
                <button className="group-action-button" onClick={handleInfo}>
                  <Icon name="info" className="group-action-icon" />
                  <span>Info</span>
                </button>
              )}
