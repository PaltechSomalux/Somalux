...
export const Memes = () => {
  ...
  const [fileSize, setFileSize] = useState(null); // State for file size feedback
  ...
  const handleDrag = useCallback(
    throttle((e) => {
      ...
      if (dragOffset.corner) {
        ...
        if (activeElement.type === 'text') {
          setTexts((prev) => prev.map((text) => (text.id === activeElement.id ? { ...text, x, y } : text)));
        } else if (activeElement.type === 'shape') {
          setShapes((prev) => prev.map((shape) => (shape.id === activeElement.id ? { ...shape, x, y } : shape)));
        } else if (activeElement.type === 'emoji') {
          setEmojis((prev) => prev.map((emoji) => (emoji.id === activeElement.id ? { ...emoji, x, y } : emoji)));
        } else if (activeElement.type === 'photo') {
          setPhotos((prev) => prev.map((photo) => (photo.id === activeElement.id ? { ...photo, x, y } : photo)));
        }
      }
    }, 8),
    [dragging, activeElement, dragOffset, texts, shapes, emojis, photos, restrictToBounds, updateShape, measureTextWidth]
  );
  ...
  const handleImageUpload = useCallback((e) => {
    ...
    reader.onload = (event) => {
      ...
      setFileSize(null); // Reset file size feedback
    };
    reader.readAsDataURL(file);
  }, []);
  ...
  const downloadMeme = useCallback(() => {
    ...
    if (!canvasRef?.current || !imageRef?.current || !image) {
      console.error('Canvas or image not available for download');
      return;
    }
    ...
    Promise.all([loadImage(image), ...photos.map((photo) => loadImage(photo.src))])
      .then(([backgroundImg, ...photoImages]) => {
        ...
        const format = 'image/png'; // Use PNG for lossless quality
        ...
        canvas.toBlob(
          (blob) => {
            ...
            setFileSize({ kb: fileSizeKB, mb: fileSizeMB });

            // Warn if file size is too large
            if (blob.size > 5 * 1024 * 1024) {
              console.warn(`Image size (${fileSizeMB} MB) is large. Consider using JPEG for smaller size.`);
            }
      ...
      <div className="editor-container">
        ...
        {image && (
          <MemeControls
            ...
            canvas={canvasRefs.canvasRef} // Pass canvasRef as canvas prop
          />
        )}
      </div>
    </div>
