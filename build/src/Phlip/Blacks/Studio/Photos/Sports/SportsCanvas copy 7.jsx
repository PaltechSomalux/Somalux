...
export const SportsCanvas = ({
  ...
  const [isPaltechLogoDeleted, setIsPaltechLogoDeleted] = useState(false); // New state to track deletion
  ...
  useEffect(() => {
    if (
      ...
      !isPaltechLogoDeleted // Only add if not deleted
    ) {
  ...
  }, [image, imageDimensionsLocal, logos, setLogos, selectedFeature, additionalImages, isMatchday, isPaltechLogoDeleted]);
  ...
  // New function to handle logo deletion
  const handleDeleteLogo = useCallback(
    (logoId) => {
      setLogos((prevLogos) => prevLogos.filter((logo) => logo.id !== logoId));
      if (logoId === 'paltechWhite') {
        setIsPaltechLogoDeleted(true); // Mark Paltech logo as deleted
      }
      setActiveElement({ type: null, id: null }); // Deselect the logo
    },
    [setLogos, setIsPaltechLogoDeleted, setActiveElement]
  );
  ...
  const customHandleTextKeyDown = useCallback(
    (id, e) => {
      if (e.key === 'Enter' && e.shiftKey) {
        ...
        updateText(id, {
          content: newContent,
          color: newContent === 'Tap to edit' ? '#FF0000' : textColor,
        });
        setIsEditing(false);
      }
    },
    [updateText, textColor, setIsEditing]
  );
    ...
    >
      ...
      >
        ...
        <SportsTextElements
          ...
          handleTextKeyDown={customHandleTextKeyDown}
