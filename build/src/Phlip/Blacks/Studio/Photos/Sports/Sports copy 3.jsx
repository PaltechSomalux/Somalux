...
export const Sports = () => {
  ...
  const [additionalImages, setAdditionalImages] = useState([]); // Replaced secondImage with additionalImages
  ...
  const [canvasRefs, setCanvasRefs] = useState({ canvasRef: null, imageRef: null, additionalImageRefs: [] }); // Updated to include additionalImageRefs
  ...
  const handleImageUpload = useCallback((e) => {
    ...
    reader.onload = (event) => {
      ...
      setAdditionalImages([]); // Reset additional images on new primary image upload
      setTexts([]);
      setLogos([]);
      setTeam1Logo(null);
      setTeam2Logo(null);
  ...
  const handleAdditionalImageUpload = useCallback((e) => {
    const file = e.target.files[0];
    if (!file || additionalImages.length >= 3) return; // Limit to 3 additional images
  ...
  const addText = useCallback(() => {
    ...
    const newText = {
      ...
      type: 'text',
      content: 'Tap to edit',
      x: additionalImages.length > 0 ? x / 2 : x, // Adjusted for additional images
      y,
      fontFamily,
      fontSize: 40,
      color: '#FF0000',
    ...
    setIsEditing(true);
  }, [fontFamily, strokeColor, textStyles, canvasRefs, logos, additionalImages.length]);
  ...
  const addLogo = useCallback(
    ...
    [logoOpacity, logoBrightness, canvasRefs, logos, additionalImages.length]
  );
  ...
  const handleLogoResize = useCallback(
    throttleEvent((id, e) => {
      ...
      const { x, y, width, height } = restrictToBounds('logo', id, logo.x, y, newWidth, newHeight, logo.rotation, canvasRefs.canvasRef || canvasRefs.imageRef, logos);
  ...
  return (
    <div className="sports-editor">
      <div className="editor-container">
        <SportsCanvas
          image={image}
          additionalImages={additionalImages} // Replaced secondImage
          setAdditionalImages={setAdditionalImages} // Replaced setSecondImage
          handleAdditionalImageUpload={handleAdditionalImageUpload} // Replaced handleSecondImageUpload
          texts={texts}
          setTexts={setTexts}
          logos={logos}
          setLogos={setLogos}
