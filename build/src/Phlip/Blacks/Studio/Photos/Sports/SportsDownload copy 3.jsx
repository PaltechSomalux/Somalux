...
export const SportsDownload = ({
  ...
  useEffect(() => {
    const preloadImages = async () => {
      try {
        ...
        // Preload second background image only if isMatchday is true and secondImage exists
        if (isMatchday && secondImage) {
  ...
  const downloadSportsImage = useCallback(() => {
    ...
    const singleImageHeight = img.naturalHeight;
    const singleImageWidth = img.naturalWidth;
    const aspectRatio = singleImageWidth / singleImageHeight;

    // Set canvas to square: width = height
    let originalWidth = singleImageHeight; // Base width on height for square
    let originalHeight = singleImageHeight;
    if (isMatchday && secondImage) {
      // Double the width for side-by-side images
      originalWidth = singleImageHeight * 2;
    } else {
      // Adjust single image to fit square canvas
      originalWidth = singleImageHeight;
    }
    ...
    // Cache text measurements to avoid redundant calls
    ...
    `;
    if (isMatchday && secondImage) {
      // Draw first image in left half
      tempCtx.drawImage(img, 0, 0, originalWidth / 2, originalHeight);
    } else {
      // Draw first image to fill canvas, maintaining aspect ratio
      const drawWidth = originalHeight * aspectRatio > originalWidth ? originalWidth : originalHeight * aspectRatio;
      const drawHeight = originalHeight * aspectRatio > originalWidth ? originalWidth / aspectRatio : originalHeight;
      const offsetX = (originalWidth - drawWidth) / 2;
      const offsetY = (originalHeight - drawHeight) / 2;
      tempCtx.drawImage(img, offsetX, offsetY, drawWidth, drawHeight);
    }
    tempCtx.filter = 'none';

    // Draw second background image only if isMatchday is true and secondImage exists
    if (isMatchday && secondImage && secondImageRef?.current) {
      ...
      tempCtx.filter = `
        ...
        blur(${filters.blur}px)
      `;
      // Draw second image in right half
      tempCtx.drawImage(secondImg, originalWidth / 2, 0, originalWidth / 2, originalHeight);
      tempCtx.filter = 'none';
    }
    ...
    const scoreBoxLeft = (originalWidth - scoreBoxWidth) / 2;
    ...
    tempCtx.fillText(competitionText, originalWidth / 2, competitionY + competitionHeight / 2, scoreBoxWidth * 0.5);
    tempCtx.restore();
    ...
    const teamLogoWidth = (isMatchday ? 70 : 60) * scaleX;
    const scoreContainerWidth = (isMatchday ? 100 : 150) * scaleX;
    ...
    if (team1Logo) {
      ...
      const team1Y = logosScoresY + (isMatchday ? 70 : 60) / 2 * scaleY;
    ...
    if (team2Logo) {
      ...
      const team2Y = logosScoresY + (isMatchday ? 70 : 60) / 2 * scaleY;
      teamLogoPromises.push(
        drawLogo(
          tempCtx,
    ...
    if (isMatchday) {
      // Handle the matchStatus format: "weekday day/month hour:minute"
      ...
      tempCtx.fillText(statusText, originalWidth / 2, matchStatusY + matchStatusHeight / 2);
      tempCtx.restore();
    }
    ...
    if (!isMatchday) {
      ...
      team2Goals.forEach((goal, index) => {
        ...
        let xOffset = (originalWidth - scoreBoxWidth) / 2 + team2Offset + (teamGoalsWidth - entryWidth) / 2;
    ...
    texts.forEach((text) => {
      ...
      if (!textWidth) {
        textWidth = tempCtx.measureText(text.content || '').width;
