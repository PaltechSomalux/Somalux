...
export const SportsDownload = ({
  ...
  const downloadSportsImage = useCallback(() => {
    ...
    // Fixed download size: 2048x2048 pixels
    const downloadWidth = 2048;
    const downloadHeight = 2048;
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5);
    tempCanvas.width = downloadWidth * dpr;
    tempCanvas.height = downloadHeight * dpr;
    tempCtx.scale(dpr, dpr);

    // Scale factor based on the canvas dimensions
    const canvasWidth = imageDimensions.width;
    const canvasHeight = imageDimensions.height;
    const scaleX = downloadWidth / canvasWidth;
    const scaleY = downloadHeight / canvasHeight;

    const img = imageCache.current.get(image) || imageRef.current;
    const aspectRatio = img.naturalWidth / img.naturalHeight;

    // Draw single image (non-matchday or if secondImage is absent)
    if (!isMatchday || !secondImage) {
      const drawWidth = downloadHeight * aspectRatio;
      const offsetX = (downloadWidth - drawWidth) / 2;
      tempCtx.filter = `
      ...
      `;
      tempCtx.drawImage(img, offsetX, 0, drawWidth, downloadHeight);
      tempCtx.filter = 'none';
    } else {
      // Draw two images side by side for matchday
      const secondImg = imageCache.current.get(secondImage) || secondImageRef.current;
      const halfWidth = downloadWidth / 2; // 1024 pixels each

      // Scale and draw first image
      const firstAspectRatio = img.naturalWidth / img.naturalHeight;
      const firstDrawWidth = halfWidth * firstAspectRatio;
      const firstDrawHeight = downloadHeight;
      tempCtx.filter = `
        brightness(${filters.brightness}%)
        contrast(${filters.contrast}%)
        saturate(${filters.saturation}%)
        ...
        blur(${filters.blur}px)
      `;
      tempCtx.drawImage(img, 0, 0, halfWidth, firstDrawHeight);
      tempCtx.filter = 'none';

      // Scale and draw second image
      const secondAspectRatio = secondImg.naturalWidth / secondImg.naturalHeight;
      const secondDrawWidth = halfWidth * secondAspectRatio;
      const secondDrawHeight = downloadHeight;
      tempCtx.filter = `
        brightness(${filters.brightness}%)
        contrast(${filters.contrast}%)
        saturate(${filters.saturation}%)
        ...
        blur(${filters.blur}px)
      `;
      tempCtx.drawImage(secondImg, halfWidth, 0, halfWidth, secondDrawHeight);
      tempCtx.filter = 'none';
    }
    ...
    const scoreBoxWidth = downloadWidth * 0.8; // 1638.4 pixels
    ...
    const scoreBoxY = downloadHeight - scoreBoxHeight - 10 * scaleY;
    const scoreBoxLeft = (downloadWidth - scoreBoxWidth) / 2;
    ...
    tempCtx.fillText(competitionText, downloadWidth / 2, competitionY + competitionHeight / 2, scoreBoxWidth * 0.5);
    tempCtx.restore();
    ...
    if (isMatchday) {
      let formattedMatchStatus = matchStatus || 'Sun 17/Aug 09:59';
      ...
      tempCtx.fillText(statusText, downloadWidth / 2, matchStatusY + matchStatusHeight / 2);
      tempCtx.restore();
    }
    ...
    if (!isMatchday) {
      ...
      team2Goals.forEach((goal, index) => {
        ...
        let xOffset = (downloadWidth - scoreBoxWidth) / 2 + team2Offset + (teamGoalsWidth - entryWidth) / 2;
