...
export const SportsCanvas = ({
  ...
  // New state for score box visibility
  const [isScoreBoxVisible, setIsScoreBoxVisible] = useState(true);
  // State to track touch start position for swipe detection
  ...
  // Handle touch start to capture initial Y position
  const handleTouchStart = useCallback((e) => {
    if (isMatchday && secondImage) {
      ...
      // Check if touch is in the middle third of the canvas (horizontally)
  ...
  }, [isMatchday, secondImage]);

  // Handle touch move to detect swipe direction
  const handleTouchMoveSwipe = useCallback(
    (e) => {
      if (touchStartY !== null && isMatchday && secondImage) {
        const touch = e.touches[0];
        const canvas = canvasRef.current;
        const rect = canvas.getBoundingClientRect();
        const touchX = touch.clientX - rect.left;
        ...
        // Ensure touch is still in the middle third
        const middleThirdStart = canvas.width / 3;
        const middleThirdEnd = (canvas.width * 2) / 3;
        if (touchX >= middleThirdStart && touchX <= middleThirdEnd) {
          ...
          if (Math.abs(deltaY) > swipeThreshold) {
            if (deltaY > 0) {
              // Swipe down - hide score box
              setIsScoreBoxVisible(false);
            } else {
              // Swipe up - show score box
              setIsScoreBoxVisible(true);
            }
            setTouchStartY(null); // Reset to prevent multiple triggers
          }
        }
      }
      // Call existing handleTouchMove for other functionality
      handleTouchMove(e);
    },
    [touchStartY, isMatchday, secondImage, handleTouchMove]
  );
    ...
    if (image && !logos.some(logo => logo.id === 'paltechWhite')) {
      ...
      setLogos((prevLogos) => [
        ...prevLogos,
        {
          ...
          type: 'logo',
          src: PaltechWhite,
          x: imageDimensionsLocal.width ? (secondImage && isMatchday ? imageDimensionsLocal.width / 2 : imageDimensionsLocal.width) - logoWidth / 2 - padding : 490,
  ...
  }, [image, secondImage, isMatchday, imageDimensionsLocal, logos, setLogos]);
  ...
  useEffect(() => {
    if (image && imageRef.current && canvasRef.current) {
      ...
      const updateDimensions = () => {
        ...
        const canvasWidth = secondImage && isMatchday ? width * 2 : width;
          ...
          setLogos((prevLogos) =>
            prevLogos.map((logo) =>
              logo.id === 'paltechWhite'
                ? {
                    ...logo,
                    x: (secondImage && isMatchday ? canvasWidth / 2 : canvasWidth) - logo.width / 2 - 10,
                    y: logo.height / 2 + 10,
                  }
                : logo
            )
  ...
  const redrawCanvas = useCallback(() => {
    ...
    if (image && imageRef.current) {
      const img = imageRef.current;
      const width = secondImage && isMatchday ? canvas.width / 2 : canvas.width;
      ctx.filter = `
    ...
    if (secondImage && secondImageRef.current && isMatchday) {
      ...
      ctx.filter = `
        ...
        blur(${filters.blur}px)
      `;
      ctx.drawImage(secondImg, width, 0, width, canvas.height);
      ctx.filter = 'none';
    }
  ...
  return (
    ...
    >
      ...
      >
        {image && (
          <>
            <div style={{ width: secondImage && isMatchday ? '50%' : '100%', height: `${imageDimensionsLocal.height}px` }}>
              <img
                ref={imageRef}
                src={image}
            ...
            {secondImage && isMatchday && (
              <div style={{ width: '50%', height: `${imageDimensionsLocal.height}px` }}>
                <img
                  ref={secondImageRef}
                  src={secondImage}
        ...
        {image && secondImage && isMatchday && (
          <div
            ...
            style={{
              ...
              left: '50%',
              transform: 'translateX(-50%)',
              display: isScoreBoxVisible ? 'flex' : 'none',
              ...
              opacity: isScoreBoxVisible ? 1 : 0,
              transform: isScoreBoxVisible ? 'translateY(0)' : 'translateY(100%)',
            }}
          >
            <div
              ...
              >
                ...
                >
                  {team1Logo?.src ? (
                    ...
                    <span style={{ fontSize: '20px', color: '#666' }}>Team 1</span>
                  )}
                </button>
              </div>
              <div
              ...
              >
                ...
                >
                  {team2Logo?.src ? (
                    ...
                    <span style={{ fontSize: '20px', color: '#666' }}>Team 2</span>
                  )}
                </button>
              </div>
            </div>
              ...
              >
                ...
                >
                  {team1Logo?.src ? (
                    ...
                    <span style={{ fontSize: '16px', color: '#666' }}>Team 1</span>
                  )}
                </button>
              </div>
              <div
              ...
              >
                ...
                >
                  {team2Logo?.src ? (
                    ...
                    <span style={{ fontSize: '16px', color: '#666' }}>Team 2</span>
                  )}
                </button>
              </div>
            </div>
