...
export const Sports = () => {
  ...
  const handleMatchdayClick = useCallback(() => {
    ...
    setSelectedFeature(null); // Reset selectedFeature when toggling matchday
    if (!isMatchday) {
      setTeam1Score(0);
      setTeam2Score(0);
      setTeam1Goals([]);
  ...
  const handleQuoteClick = useCallback(() => {
    ...
    if (selectedFeature !== 'quote') {
      ...
      setMatchStatus(formattedDateTime);
      setShowScoreBox(true);
      setIsMatchday(false); // Ensure matchday is disabled when quote is enabled
    } else {
      setMatchStatus('Full-Time');
    }
  }, [selectedFeature]);
        ...
        {(image || logos.length > 0) && (
          <>
            <SportsControls
              ...
              handleQuoteClick={handleQuoteClick} // Added prop for quote feature
            />
            <SportsDownload
              image={image}
              additionalImages={additionalImages}
