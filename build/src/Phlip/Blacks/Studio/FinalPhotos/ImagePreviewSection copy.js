...
export const ImagePreviewSection = ({
  ...
  const handleMouseMove = (e) => {
    ...
    if (isDragging) {
      ...
      const newScale = Math.max(0.5, Math.min(3, activeText.scale + deltaX * 0.01)); // Scale between 0.5 and 3
  ...
  const handleZoom = (direction) => {
    ...
    const newScale = Math.max(0.5, Math.min(3, activeText.scale + scaleChange));
    updateTextElement(activeTextId, { scale: newScale });
  };
  ...
  }, [isDragging, isRotating, isResizing, activeTextId, textElements]);
