...
const ReadingCharts = ({ stats, userId }) => {
  ...
  const monthlyData = (monthly || []).map(item => ({
    month: item.month_name || `${item.month}/${item.year}`,
            ...
            <div key={index} className="leaderboard-item">
              ...
              <div className="leaderboard-avatar">
                {user.profiles?.avatar_url ? (
                  <img src={user.profiles.avatar_url} alt={user.profiles.username} />
                ) : (
                  <div className="avatar-placeholder">{user.profiles?.username?.[0]?.toUpperCase()}</div>
                )}
              </div>
              <div className="leaderboard-name">{user.profiles?.username || 'Anonymous'}</div>
