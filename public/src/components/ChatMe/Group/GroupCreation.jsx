...
export const GroupCreation = ({ onClose, onGroupCreated }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        // Query users table, filtering to only creator's friends
        ...
        console.log('Available users to add (creator friends only):', usersData?.length || 0);
        setUsers(usersData || []);
      } catch (err) {
            ...
            <div className="users-list">
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map(user => (
                  ...
                  >
                    <img
                      src={user.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.full_name || user.email)}&size=48`}
                      alt={user.full_name}
                      className="user-avatar"
                    />
                    <div className="user-info">
                      <div className="user-name">{user.full_name || user.email?.split('@')[0] || 'Unknown User'}</div>
              ...
              <div className="members-avatars">
                ...
                {users.filter(u => selectedUsers.includes(u.id)).slice(0, 4).map(user => (
                  <div key={user.id} className="avatar-wrapper">
                    <img
                      src={user.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.full_name || user.email)}&size=40`}
                      alt={user.full_name}
                      className="member-avatar"
                      title={user.full_name || user.email}
                      onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.insertAdjacentHTML('afterend', `<div class="member-avatar-fallback">${(user.full_name || user.email || 'U').charAt(0).toUpperCase()}</div>`); }}
                    />
                  </div>
                ))}
