...
const Comment = ({ comment, mediaId, currentUser, onDelete, onLikeToggle, onReply }) => {
  ...
  const handleReplySubmit = async (e) => {
    ...
    const newReply = {
      ...
      media: replyMedia
        ? {
            type: replyMedia.type.startsWith('image') ? 'image' : 'video',
    ...
    >
      ...
      <AnimatePresence>
        {Array.isArray(comment.replies) && comment.replies.length > 0 && (
          <div className="replies-container-comments">
            {comment.replies.map((reply) => (
              <motion.div
                key={reply.id}
                initial={{ opacity: 0, x: -10 }}
                animate={{ opacity: 1, x: 0 }}
                className="reply-comments"
              >
...
export const CommentsSection = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const commentData = {
      ...
      media: attachedMedia
        ? {
            type: attachedMedia.type.startsWith('image') ? 'image' : 'video',
