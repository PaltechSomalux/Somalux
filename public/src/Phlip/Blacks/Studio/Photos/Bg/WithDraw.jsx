...
export const Memes = () => {
  ...
  const startDrawing = useCallback((e) => {
    if (!image || selectedFeature !== 'draw') return;
    ...
    setCurrentDrawing({
      ...
      width: drawingMode === 'pen' ? drawingWidth : 
            drawingMode === 'brush' ? brushSize : 
            eraserSize,
      mode: drawingMode
    });
  }, [image, selectedFeature, drawingMode, drawingColor, drawingWidth, brushSize, eraserSize]);
  ...
  // Redraw all drawings when needed
  ...
  useEffect(() => {
    redrawDrawings();
  }, [drawings, currentDrawing, redrawDrawings]);
  ...
  const handleElementClick = useCallback(
    ...
    [texts, shapes, emojis, photos, isCropping, isDrawing]
  );
  ...
  const handleDrag = useCallback(
    throttle((e) => {
      if (!dragging || !activeElement.id || isDrawing) return;
      e.preventDefault();
      const clientX = e.clientX || (e.touches && e.touches[0]?.clientX) || 0;
      const clientY = e.clientY || (e.touches && e.touches[0]?.clientY) || 0;
    ...
    [dragging, activeElement, dragOffset, isDrawing]
  );
  ...
  const startRotation = useCallback(
    ...
    [texts, shapes, emojis, photos, isCropping, isDrawing, cancelLongPress]
  );

  const handleRotation = useCallback(
    throttle((e) => {
      if (!isRotating || !activeElement.id || isDrawing) return;
      e.preventDefault();
      const clientX = e.clientX || (e.touches && e.touches[0]?.clientX) || 0;
      const clientY = e.clientY || (e.touches && e.touches[0]?.clientY) || 0;
    ...
    [isRotating, activeElement, rotationCenter, rotationStartAngle, updateText, updateShape, updateEmoji, updatePhoto, isDrawing]
  );
  ...
  const handleMouseWheelRotation = useCallback(
    ...
    [activeElement, texts, shapes, emojis, photos, isCropping, isDrawing, updateText, updateShape, updateEmoji, updatePhoto]
  );

  const handleTextZoom = useCallback(
    throttle((id, e) => {
      if (activeElement.type !== 'text' || activeElement.id !== id || isDrawing) return;
    ...
    [activeElement, texts, updateText, isDrawing]
  );

  const handleShapeResize = useCallback(
    throttle((id, e) => {
      if (activeElement.type !== 'shape' || activeElement.id !== id || isDrawing) return;
      const delta = e.deltaY || 0;
      const resizeStep = 1;
      const minSize = 20;
      const maxSize = 500;
    ...
    [activeElement, shapes, updateShape, isDrawing]
  );

  const handleEmojiZoom = useCallback(
    throttle((id, e) => {
      if (activeElement.type !== 'emoji' || activeElement.id !== id || isDrawing) return;
      const delta = e.deltaY || 0;
      const zoomStep = 0.5;
      const minSize = 10;
      const maxSize = 200;
    ...
    [activeElement, emojis, updateEmoji, isDrawing]
  );

  const handlePhotoResize = useCallback(
    throttle((id, e) => {
      if (activeElement.type !== 'photo' || activeElement.id !== id || isDrawing) return;
      const delta = e.deltaY || 0;
      const resizeStep = 1;
      const minSize = 20;
      const maxSize = 500;
    ...
    [activeElement, photos, updatePhoto, isDrawing]
  );
  ...
  const handleTouchMove = useCallback(
    throttle((e) => {
      if (e.touches.length === 2 && initialDistance !== null && !isDrawing) {
        e.preventDefault();
        const touch1 = e.touches[0];
        const touch2 = e.touches[1];
    ...
    [initialDistance, initialFontSize, activeElement, updateText, updateShape, updateEmoji, updatePhoto, handleDrag, isDrawing]
  );
    ...
    Promise.all([loadImage(image), ...photos.map((photo) => loadImage(photo.src))])
      .then(([backgroundImg, ...photoImages]) => {
        ...
        drawings.forEach(drawing => {
          ...
          ctx.strokeStyle = drawing.color;
          ctx.lineWidth = drawing.width * Math.max(scaleX, scaleY);
          ctx.lineCap = 'round';
          ctx.lineJoin = 'round';
          ctx.stroke();
        });
  ...
  }, [image, texts, shapes, emojis, photos, filters, drawings]);
    ...
    return photos.map((photo) => (
      <div
        ...
        style={{
          ...
          pointerEvents: isCropping || isDrawing ? 'none' : 'auto',
        }}
      >
        <img
          src={photo.src}
    ...
    <div className="meme-editor">
      <div className="editor-container">
        <div
          ...
          ref={wrapperRef}
          onPointerMove={(e) => {
            if (isCropping) handleCropDrag(e);
            else if (isDrawing) draw(e);
            else handleDrag(e);
          }}
          onPointerUp={isCropping ? stopCrop : isDrawing ? endDrawing : stopDragging}
          onPointerLeave={isCropping ? stopCrop : isDrawing ? endDrawing : stopDragging}
          onTouchMove={(e) => {
            if (isCropping) handleCropDrag(e);
            else if (isDrawing) draw(e);
            else handleTouchMove(e);
          }}
          onTouchEnd={isCropping ? stopCrop : isDrawing ? endDrawing : handleTouchEnd}
              ...
              <div className="image-container" style={{ position: 'relative' }}>
                ...
                <canvas
                  ...
                  style={{
                    ...
                    width: '100%',
                    height: '100%',
                    pointerEvents: selectedFeature === 'draw' ? 'auto' : 'none',
                  }}
              ...
              <div className="bottom-toolbar-memes">
                <div className="primary-tools-scroll-container-memes">
                  <div className="tool-group-memes primary-tools">
                    ...
                    <button
                      className={`tool-button-memes ${selectedFeature === 'draw' ? 'active' : ''}`}
                      onClick={() => {
                        setSelectedFeature('draw');
                        setActiveElement({ type: null, id: null });
                      }}
                      data-tooltip="Draw on your meme"
                      disabled={isCropping}
                    >
                      <span>Draw</span>
                    </button>
