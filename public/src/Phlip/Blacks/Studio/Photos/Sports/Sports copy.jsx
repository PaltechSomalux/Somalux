...
export const Sports = () => {
  ...
  const [canvasRefs, setCanvasRefs] = useState({ canvasRef: null, imageRef: null, secondImageRef: null });
  ...
  const handleSecondImageUpload = useCallback((e) => {
    const file = e.target.files[0];
    if (!file) return;
  ...
  const addText = useCallback(() => {
    ...
    const newText = {
      ...
      type: 'text',
      content: 'Tap to edit',
      x: isMatchday ? x / 2 : x, // Adjust x position for matchday
      y,
      fontFamily,
      fontSize: 40,
      color: '#FF0000',
    ...
    setIsEditing(true);
  }, [fontFamily, strokeColor, textStyles, canvasRefs, logos, isMatchday]);
  ...
  const addLogo = useCallback(
    (e) => {
      ...
      reader.onload = (event) => {
        ...
        setLogos((prev) => [...prev, { ...newLogo, x: boundedX, y: boundedY, width, height }]);
        setActiveElement({ type: 'logo', id: newLogo.id });
        setSelectedFeature('logo');
      };
      reader.readAsDataURL(file);
    },
    [logoOpacity, logoBrightness, canvasRefs, logos, isMatchday]
  );
  ...
  const handleStartDragging = useCallback(
    (type, id, e) => {
      ...
      const { x, y } = restrictToBounds(
        ...
        canvasRefs.canvasRef || canvasRefs.imageRef,
        logos,
        isMatchday ? imageDimensions.width / 2 : imageDimensions.width // Restrict to left half if matchday
      );
      setDragOffset({ x: clientX - x, y: clientY - y });
    },
    [texts, logos, canvasRefs, imageDimensions, isMatchday]
  );
  ...
  const handleDrag = useCallback(
    ...
    [dragging, activeElement, dragOffset, texts, logos, updateText, updateLogoProperties, canvasRefs, imageDimensions, isMatchday]
  );
  ...
  const handleTextZoom = useCallback(
    ...
    [activeElement, texts, updateText, canvasRefs, logos, imageDimensions, isMatchday]
  );
  ...
  const handleLogoResize = useCallback(
    ...
    [activeElement, logos, updateLogoProperties, canvasRefs, imageDimensions, isMatchday]
  );
  ...
  const handleTouchMove = useCallback(
    ...
    [initialDistance, initialFontSize, activeElement, updateText, updateLogoProperties, handleDrag, canvasRefs, texts, logos, imageDimensions, isMatchday]
  );
  ...
  const handleTeamTemplateSelect = useCallback(
    ...
    [logoOpacity, logoBrightness, showTeamTemplate.team]
  );
  ...
  const handleMatchdayClick = useCallback(() => {
    ...
    if (!isMatchday) {
      ...
      setTeam2Goals([]);
      const now = new Date();
      const formattedDateTime = `${now.toLocaleString('en-US', { weekday: 'short' })} ${now.getDate()}/${now.toLocaleString('en-US', { month: 'short' })} ${now.getHours()}:${now.getMinutes().toString().padStart(2, '0')}`;
      setMatchStatus(formattedDateTime);
    } else {
      setSecondImage(null); // Clear second image when exiting matchday
      setMatchStatus('Full-Time');
    }
  }, [isMatchday]);
    ...
    <div className="sports-editor">
      <div className="editor-container">
        <SportsCanvas
          ...
          handleSecondImageUpload={handleSecondImageUpload}
          texts={texts}
          setTexts={setTexts}
          logos={logos}
          setLogos={setLogos}
