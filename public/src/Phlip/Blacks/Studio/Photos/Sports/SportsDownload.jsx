...
export const SportsDownload = ({
  ...
  const drawQuoteBox = (ctx, width, height, scaleX, scaleY) => {
    ...
    ctx.font = `bold ${32 * Math.min(scaleX, scaleY)}px Arial`;
    ctx.fillStyle = 'red';
    ctx.textAlign = 'center';
    ...
    if (quoteImage) {
      ...
      drawLogo(
        ...
        scaleX,
        scaleY,
        (boxX + (boxWidth * 0.75)) / scaleX,
        (boxY - 100 * scaleY) / scaleY,
        100
      );
    }

    const quoteText = customMatchTime || 'Enter quote';
    let fontSize = 18 * Math.min(scaleX, scaleY);
    const maxWidth = boxWidth - 2 * padding;
    const lineHeight = 20 * Math.min(scaleX, scaleY);
    const availableHeight = boxHeight - 40 * scaleY - 36 * scaleY;
    ...
    while (lines.length > maxLines || textHeight > availableHeight) {
      fontSize *= 0.9;
      if (fontSize < 8 * Math.min(scaleX, scaleY)) break;
      lines = wrapText(quoteText, fontSize);
      textHeight = lines.length * lineHeight;
    }

    const textBlockHeight = lines.length * lineHeight;
    const startY = boxY + 40 * scaleY + (availableHeight - textBlockHeight) / 2;
