...
export const SportsCanvas = ({
    ...
    if (image && !logos.some(logo => logo.id === 'paltechWhite')) {
      ...
      setLogos((prevLogos) => [
        ...prevLogos,
        {
          ...
          type: 'logo',
          src: PaltechWhite,
          x: imageDimensionsLocal.width ? imageDimensionsLocal.width - logoWidth / 2 - padding : 490,
  ...
  useEffect(() => {
    if (image && imageRef.current && canvasRef.current) {
      ...
      const updateDimensions = () => {
        ...
        // Double canvas width if isMatchday and secondImage exists
        const totalCanvasWidth = isMatchday && secondImage ? canvasWidth * 2 : canvasWidth;

        setImageDimensionsLocal({ width: totalCanvasWidth, height: canvasHeight });
        setImageDimensions({ width: totalCanvasWidth, height: canvasHeight });

        canvas.width = totalCanvasWidth;
        canvas.height = canvasHeight;

        // Adjust logo positions if not moved
        if (!logoMoved) {
          setLogos((prevLogos) =>
            prevLogos.map((logo) =>
              logo.id === 'paltechWhite'
                ? {
                    ...logo,
                    x: totalCanvasWidth - logo.width / 2 - 10,
                    y: logo.height / 2 + 10,
                  }
                : logo
            )
  ...
  const redrawCanvas = useCallback(() => {
    ...
    const imageWidth = isMatchday && secondImage ? canvas.width / 2 : canvas.width;
    ...
    if (image && imageRef.current) {
      ...
      if (img.complete) {
        ...
        `;
        // Draw first image on the left half if secondImage exists and isMatchday
    ...
    if (secondImage && secondImageRef.current && isMatchday) {
      ...
      if (secondImg.complete) {
        ctx.filter = `
          ...
          blur(${filters.blur}px)
        `;
        // Draw second image on the right half
        ctx.drawImage(secondImg, imageWidth, 0, imageWidth, imageHeight);
        ctx.filter = 'none';
      }
    }

    // Draw all logos, including paltechWhite, on the canvas to ensure they are on top
    logos.forEach((logo) => {
      if (logo.type === 'logo' && logo.id !== 'team1Logo' && logo.id !== 'team2Logo') {
  ...
  return (
    ...
    >
      <div className="image-container" style={{ position: 'relative', display: 'flex', flexDirection: 'row', width: '100%' }}>
        {image && (
          <div style={{ width: isMatchday && secondImage ? '50%' : '100%', height: 'auto' }}>
            <img
              ref={imageRef}
              src={image}
              ...
              style={{
                width: '100%',
                height: imageDimensionsLocal.height ? `${imageDimensionsLocal.height}px` : 'auto',
                maxHeight: '70vh',
                display: 'block',
                objectFit: 'contain',
              }}
        ...
        <canvas
          ...
          style={{
            ...
            zIndex: 10, // Ensure canvas is above images
          }}
        />
        {isMatchday && (
          <div
        ...
        />
        {logos
          .filter((logo) => logo.id !== 'team1Logo' && logo.id !== 'team2Logo' && logo.id !== 'paltechWhite')
          .map((logo) => (
            <div
              key={logo.id}
