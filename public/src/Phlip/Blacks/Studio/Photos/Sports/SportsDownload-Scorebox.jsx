...
export const SportsDownload = ({
  ...
  const downloadSportsImage = useCallback(() => {
    ...
    // Set target resolution to 1080x1350 for matchday images, otherwise use original dimensions
    const targetWidth = isMatchday && secondImage ? 1080 : originalWidth;
    const targetHeight = isMatchday && secondImage ? 1350 : originalHeight;
    ...
    const scaleX = isMatchday && secondImage ? targetWidth / (canvasWidth * 2) : targetWidth / canvasWidth;
    ...
    tempCtx.drawImage(
      ...
      0,
      0,
      isMatchday && secondImage ? targetWidth / 2 : targetWidth,
      targetHeight
    );
    tempCtx.filter = 'none';
    ...
    if (isMatchday && secondImage && secondImageRef?.current) {
      ...
      tempCtx.filter = `
        ...
        blur(${filters.blur}px)
      `;
      tempCtx.drawImage(secondImg, targetWidth / 2, 0, targetWidth / 2, targetHeight);
      tempCtx.filter = 'none';
    }
    ...
    // Initialize teamLogoPromises in the outer scope
    const teamLogoPromises = [];

    // Draw score box only if visible and (not isMatchday or isMatchday with secondImage)
    if (isScoreBoxVisible && (isMatchday ? secondImage : true)) {
      ...
      const scoreBoxHeight = isMatchday
        ...
        : (36 + 60 + 36 + numGoals * 24) * Math.min(scaleX, scaleY); // Competition + Logos/Scores + Status + Goals
      ...
      const teamLogoWidth = (isMatchday ? 80 : 60) * scaleX; // Match canvas logo size
      ...
      if (team1Logo) {
        ...
        const team1Y = logosScoresY + (isMatchday ? 80 : 60) / 2 * scaleY;
      ...
      if (team2Logo) {
        ...
        const team2Y = logosScoresY + (isMatchday ? 80 : 60) / 2 * scaleY;
        teamLogoPromises.push(
          drawLogo(
            tempCtx,
