...
import { SHAPES, EMOJIS, FONT_FAMILIES, UNDERLINE_STYLES, TEXT_COLORS, DESIGNER_COLORS } from './MemeConstants';
import './Memes.css';
...
export const MemeControls = ({
  ...
  // Memoize the merge images handler
  const handleMergeClick = useCallback(() => {
    mergeInputRef.current.click();
  }, []);

  // Memoize the template click handler
  const onTemplateClick = useCallback(() => {
    handleTemplateClick();
  }, [handleTemplateClick]);

  // Memoize the eraser click handler
  const handleEraserClick = useCallback(() => {
    setSelectedFeature('eraser');
  }, [setSelectedFeature]);
    ...
    <div className="bottom-toolbar-memes">
      <div className="primary-tools-scroll-container-memes">
        <div className="tool-group-memes primary-tools">
          ...
          <button
            className={`tool-button-memes ${selectedFeature === 'eraser' ? 'active' : ''}`}
            onClick={handleEraserClick}
            data-tooltip="Erase parts of the canvas"
            disabled={false}
      ...
      {selectedFeature && selectedFeature !== 'filters' && selectedFeature !== 'eraser' && (
