...
export const Memes = () => {
  ...
  const [textColor, setTextColor] = useState(DESIGNER_COLORS[1].value); // Midnight Charcoal
  const [strokeColor, setStrokeColor] = useState(DESIGNER_COLORS[1].value); // Midnight Charcoal
  const [shapeFillColor, setShapeFillColor] = useState(DESIGNER_COLORS[2].value); // Sunset Orange
  const [shapeOutlineColor, setShapeOutlineColor] = useState(DESIGNER_COLORS[1].value); // Midnight Charcoal
  ...
  const [isCropping, setIsCropping] = useState(false); // New cropping state
  const [eraserSettings, setEraserSettings] = useState({
    size: 10,
    opacity: 100,
    hardness: 100,
  }); // New eraser settings state
  ...
  const addText = useCallback(() => {
    ...
    setIsEditing(true);
  }, [fontFamily, strokeColor, textStyles, canvasRefs.imageRef, shapes]);
  ...
  const addEmoji = useCallback(() => {
    ...
    const { x: boundedX, y: boundedY, width: size } = restrictToBounds('emoji', newEmoji.id, x, y, emojiSize, emojiSize, 0, canvasRefs.imageRef, shapes);
  ...
  const addPhoto = useCallback(
    (e) => {
      ...
      reader.onload = (event) => {
        ...
        const { x: boundedX, y: boundedY, width, height } = restrictToBounds('photo', newPhoto.id, x, y, 100, 100, 0, canvasRefs.imageRef, shapes);
  ...
  const handleTemplateSelect = useCallback((meme) => {
    ...
    setEraserSettings({ size: 10, opacity: 100, hardness: 100 }); // Reset eraser settings
  }, []);
  ...
  const handleStartDragging = useCallback(
    ...
    [texts, shapes, emojis, photos, canvasRefs.imageRef, shapes]
  );
  ...
  const handleDrag = useCallback(
    throttleEvent((e) => {
      ...
      const { x, y } = restrictToBounds(activeElement.type, activeElement.id, newX, newY, width, height, element.rotation, canvasRefs.imageRef, shapes);
    ...
    [dragging, activeElement, dragOffset, texts, shapes, emojis, photos, updateText, updateShape, updateEmoji, updatePhotoProperties, canvasRefs.imageRef, shapes]
  );
  ...
  const handleTextZoom = useCallback(
    throttleEvent((id, e) => {
      ...
      const { x, y } = restrictToBounds('text', id, text.x, text.y, textWidth, newFontSize, text.rotation, canvasRefs.imageRef, shapes);
      updateText(id, { fontSize: newFontSize, x, y });
    }, 10),
    [activeElement, texts, updateText, canvasRefs.imageRef, shapes]
  );
  ...
  const handleShapeResize = useCallback(
    throttleEvent((id, e) => {
      ...
      const { x, y, width, height } = restrictToBounds('shape', id, shape.x, shape.y, newWidth, newHeight, shape.rotation, canvasRefs.imageRef, shapes);
      updateShape(id, { width, height, x, y });
    }, 10),
    [activeElement, shapes, updateShape, canvasRefs.imageRef]
  );
  ...
  const handleEmojiZoom = useCallback(
    throttleEvent((id, e) => {
      ...
      const { x, y, width: size } = restrictToBounds('emoji', id, emoji.x, emoji.y, newSize, newSize, emoji.rotation, canvasRefs.imageRef, shapes);
      updateEmoji(id, { size, x, y });
    }, 10),
    [activeElement, emojis, updateEmoji, canvasRefs.imageRef, shapes]
  );
  ...
  const handlePhotoResize = useCallback(
    throttleEvent((id, e) => {
      ...
      const { x, y, width, height } = restrictToBounds('photo', id, photo.x, photo.y, newWidth, newHeight, photo.rotation, canvasRefs.imageRef, shapes);
      updatePhotoProperties(id, { width, height, x, y });
    }, 10),
    [activeElement, photos, updatePhotoProperties, canvasRefs.imageRef, shapes]
  );
  ...
  const handleTouchMove = useCallback(
    throttleEvent((e) => {
      if (initialDistance !== null && e.touches.length === 2) {
        ...
        if (activeElement.type === 'text') {
          ...
          const { x, y } = restrictToBounds('text', activeElement.id, element.x, element.y, textWidth, newSize, element.rotation, canvasRefs.imageRef, shapes);
    ...
    [initialDistance, initialFontSize, activeElement, updateText, updateShape, updateEmoji, updatePhotoProperties, handleDrag, canvasRefs.imageRef, shapes, texts]
  );
  ...
  const handleEmojiSizeChange = useCallback(
    (e) => {
      ...
      if (activeElement.type === 'emoji' && activeElement.id && selectedFeature === 'emoji') {
        ...
        if (!emoji) return;
        const { x, y, width: size } = restrictToBounds('emoji', activeElement.id, emoji.x, emoji.y, newSize, newSize, emoji.rotation, canvasRefs.imageRef, shapes);
