...
export const MemeCanvas = ({
  ...
  const [logoMoved, setLogoMoved] = useState(false); // Track if logo has been moved
  ...
  useEffect(() => {
    if (image && imageRef.current && canvasRef.current) {
      ...
      const updateDimensions = () => {
        ...
        // Update logo position only if it hasn't been moved
        if (!logoMoved) {
          setPhotos((prevPhotos) =>
  ...
  useEffect(() => {
    const handleLogoDrag = (e) => {
      if (activeElement.type === 'photo' && activeElement.id === 'paltechWhite' && dragging) {
        setLogoMoved(true);
      }
    };
        ...
        {image && (
          <img
            ...
            style={{
              ...
              display: 'block',
              objectFit: 'contain',
              opacity: 1, // Changed from 0 to 1 to prevent blinking
            }}
          />
        )}
        <canvas
