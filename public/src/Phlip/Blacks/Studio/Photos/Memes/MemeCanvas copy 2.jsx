...
export const MemeDownload = ({ image, texts, shapes, emojis, photos, filters, canvasRefs }) => {
  ...
  const downloadMeme = useCallback(() => {
    ...
    // Get dimensions: use canvas dimensions if no single image, or image dimensions
    ...
    if (image && imageRef?.current) {
      ...
      // Use canvas dimensions for merged photos
      originalWidth = canvas.width;
      originalHeight = canvas.height;
    }

    // Use device pixel ratio for high-resolution output
    ...
    // Draw background: either the single image or merged photos
    if (image && imageRef?.current && imageRef.current.complete) {
      ...
      `;
      tempCtx.drawImage(imageRef.current, 0, 0, originalWidth, originalHeight);
      tempCtx.filter = 'none';
    }

    // Draw photos (for both single image and merged photos cases)
  ...
  return (
    <button onClick={downloadMeme} disabled={!image && photos.length === 0} className="download-button" aria-label="Download meme">
      Download
    </button>
  );
}; 