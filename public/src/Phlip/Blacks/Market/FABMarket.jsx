...
import './FABMarket.css';
import { PollModal } from './PollModal';

export const FABMarket = ({
  onPollCreate,
  onTextCreate,
  onMediaCreate,
  isChatSelected,
  ...
  // Handle poll objects coming from the Cult PollModal
  const handlePollFromCult = (poll) => {
    try {
      if (!poll) return;
      ...
      console.error('FABMarket.jsx: Error handling poll from Cult PollModal:', err);
    } finally {
      resetModal();
    }
  };
  ...
  return (
    <div className="fab-container-mkt">
      <button
        className={`fab-main-mkt ${isOpen ? 'open' : ''}`}
        onClick={toggleMenu}
      ...
      {isOpen && (
        <div className="fab-menu-mkt">
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('poll')}>
            <FiBarChart2 className="fab-icon-mkt" />
            <span className="fab-label-mkt">Poll</span>
          </button>
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('text')}>
            <FiType className="fab-icon-mkt" />
            <span className="fab-label-mkt">Text</span>
          </button>
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('camera')}>
            <FiCamera className="fab-icon-mkt" />
            <span className="fab-label-mkt">Camera</span>
          </button>
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('image')}>
            <FiImage className="fab-icon-mkt" />
            <span className="fab-label-mkt">Image</span>
          </button>
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('video')}>
            <FiVideo className="fab-icon-mkt" />
            <span className="fab-label-mkt">Video</span>
          </button>
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('audio')}>
            <FiMic className="fab-icon-mkt" />
            <span className="fab-label-mkt">Audio</span>
          </button>
          <button className="fab-menu-item-mkt" onClick={() => handleModalOpen('document')}>
            <FiFile className="fab-icon-mkt" />
            <span className="fab-label-mkt">Document</span>
          </button>
        </div>
      )}
      {activeModal === 'poll' ? (
        ...
        <PollModal
          onClose={resetModal}
          onCreatePoll={handlePollFromCult}
          currentUser={undefined}
        />
      ) : activeModal ? (
        <div className="fab-modal-overlay-mkt">
          <div className="fab-modal-mkt" ref={modalRef}>
            <div className="fab-modal-header-mkt">
              <h3>{modalTitle}</h3>
              <button
                className="fab-modal-close-mkt"
                onClick={resetModal}
            ...
            <div className="fab-modal-content-mkt">
              {error && <p className="error-message-mkt">{error}</p>}
              {isProcessing && <p className="processing-message-mkt">Processing files...</p>}
              ...
              {['camera', 'image', 'video', 'audio', 'document'].includes(activeModal) && (
                <div className="media-upload-mkt">
                  ...
                  {mediaFiles.length > 0 && (
                    <div className="media-preview-mkt">
                      <h4>Uploaded Files ({mediaFiles.length}):</h4>
                      {mediaFiles.map((file, index) => {
                        const fileContent = fileContents[index];
                        return (
                          <div key={index} className="media-preview-item-mkt">
                            <div className="file-info-mkt">
                              {file.type.startsWith('image/') ? (
                                <img
                                  src={URL.createObjectURL(file)}
                                  alt="Preview"
                                  className="media-preview-img-mkt"
                                />
                              ) : file.type.startsWith('video/') ? (
                                <video
                                  src={URL.createObjectURL(file)}
                                  className="media-preview-video-mkt"
                                  controls
                                />
                              ) : file.type.startsWith('audio/') ? (
                                <audio
                                  src={URL.createObjectURL(file)}
                                  className="media-preview-audio-mkt"
                                  controls
                                />
                              ) : (
                                <div className="document-preview-mkt">
                                  {getMediaIcon(file)}
                                  <div className="file-details-mkt">
                                    <span className="file-name-mkt">{file.name}</span>
                                    <span className="file-size-mkt">{formatFileSize(file.size)}</span>
                                    {fileContent && (
                                      <span className="file-status-mkt">✓ Read successfully</span>
                                    )}
                                  </div>
                                </div>
                              )}
                              <button
                                className="remove-media-btn-mkt"
