...
export async function fetchUniversities({ page = 1, pageSize = 10, search = '', sort = { col: 'created_at', dir: 'desc' }, forceRefresh = false }) {
  ...
  try {
    let query = supabase
      .from('universities')
      .select('id, name, description, website_url, cover_image_url, location, established, student_count, views, created_at, uploaded_by', { count: 'exact' })
...
export async function deleteUniversity({ id, cover_image_url }) {
  // Use backend service-role endpoint to bypass RLS and rely on ON DELETE CASCADE
  const { data: { session } = {} } = await supabase.auth.getSession();

  const res = await fetch(`/api/elib/universities/${id}`, {
    method: 'DELETE',
    headers: {
  ...
  if (!res.ok) {
    ...
    const message = body?.error || `Delete failed with status ${res.status}`;
    throw new Error(message);
  }

  // Try to delete the cover image from storage (best-effort, not critical)
  if (cover_image_url) {
    const path = cover_image_url.split('/').pop();
...
export async function trackUniversityView(universityId) {
  ...
  try {
    ...
    await supabase.rpc('increment_university_views', { university_id: universityId });
  } catch (error) {
    // Ignore duplicate view errors
    console.debug('View tracking error (may be duplicate):', error);
  }
}
