...
import { subscribeToGroupTopic } from '../KissMe/utils/fcmTopics';
...
export const WebSocketProvider = ({ children }) => {
  ...
  const { sendJsonMessage, lastJsonMessage, readyState } = useWebSocket(WS_URL, {
    ...
    onOpen: () => {
      ...
      if (uid) {
        try {
          sendJsonMessage({ type: 'join_user', userId: uid });
          // console.log(`👤 Joined user channel for ${uid}`);
        } catch {}
      }
    },
    onClose: (event) => {
  ...
  const setCurrentUser = useCallback((user) => {
    ...
    try {
      if (user?.uid && readyState === 1) {
        sendJsonMessage({ type: 'join_user', userId: user.uid });
        console.log(`👤 Joined user channel (setCurrentUser) for ${user.uid}`);
      }
    } catch {}
  ...
  useEffect(() => {
    ...
    try {
      const q = fsQuery(collection(db, 'groups'), where('memberIds', 'array-contains', uid));
      let initialized = false;
      groupsListenerRef.current = onSnapshot(q, async (snap) => {
        ...
        if (!initialized) {
          // Seed known IDs without toasting existing memberships
          knownMemberGroupIdsRef.current = currentIds;
          initialized = true;
          return;
        }

        // Find newly added group IDs
        currentIds.forEach(async (gid) => {
          if (!knownMemberGroupIdsRef.current.has(gid)) {
            const data = snap.docs.find(d => d.id === gid)?.data() || {};
    ...
    groups.forEach(group => {
      if (!joinedGroupsRef.current.has(group.id)) {
        sendJsonMessage({
          ...
          userName: currentUserRef.current.name || currentUserRef.current.displayName || 'User',
          isGroup: true
        });
  ...
  useEffect(() => {
    ...
    switch (type) {
      case 'typing_start':
        if (data.userId !== currentUserRef.current?.uid) {
          setTypingUsers(prev => {
            const newState = {
              ...prev,
              [gid]: {
