...
export const GroupHeader = ({
  ...
  const getGroupMemberStatus = () => {
    ...
    if (enableFeatures.onlineStatus && !isSelfChat) {
      ...
      if (group.lastActivity) {
        return (
          <div className="groupHeaderChatmeGroups-status-container">
            <span className="groupHeaderChatmeGroups-last-activity">Last active {formatDate(group.lastActivity)}</span>
            <span className="groupHeaderChatmeGroups-member-count">{totalMembersText}</span>
          </div>
        );
      }
  ...
  return (
    <div className={`groupHeaderChatmeGroups-header ${currentTheme}-theme`}>
      ...
      {pinnedMessages.length > 0 && (
        <div className="groupHeaderChatmeGroups-pinned-container">
          <div className="groupHeaderChatmeGroups-pinned-header">
            <BsPinFill size={12} className="groupHeaderChatmeGroups-pinned-icon" />
            <span className="groupHeaderChatmeGroups-pinned-label">Pinned</span>
          </div>
          <div className="groupHeaderChatmeGroups-pinned-list" ref={pinnedRef}>
            {pinnedMessages.map(pin => (
              <div key={pin.id} className="groupHeaderChatmeGroups-pinned-item" role="button" tabIndex={0}>
                <div className="groupHeaderChatmeGroups-pinned-content" onClick={() => onPinClick(pin, true)}>
                  <div className="groupHeaderChatmeGroups-pinned-text">
                    {pin.text?.substring(0, 30)}{pin.text?.length > 30 ? '...' : ''}
                  </div>
                  <div className="groupHeaderChatmeGroups-pinned-time">{formatDate(pin.timestamp)}</div>
                </div>
                <button className="groupHeaderChatmeGroups-pinned-unpin" onClick={(e) => { e.stopPropagation(); onUnpinMessage(pin); }}>
                  <FiX size={14} />
                </button>
              </div>
            ))}
      ...
      <div className="groupHeaderChatmeGroups-chatMenuContainer" ref={menuRef}>
        ...
        <button className="groupHeaderChatmeGroups-iconButton groupHeaderChatmeGroups-menuButton" onClick={() => setShowChatMenu(!showChatMenu)}>
