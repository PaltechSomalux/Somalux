...
const UsersAnalytics = ({ rows }) => {
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ marginTop: 8, height: 140, background: '#111b21', borderRadius: 6, border: '1px solid #202c33', padding: 6 }}>
        {chartData && chartData.length > 0 ? (
          <ResponsiveContainer width="100%" height={144} minWidth={0}>
            <BarChart data={chartData}>
              <XAxis dataKey="label" stroke="#8696a0" tick={{ fontSize: 11 }} />
              <YAxis stroke="#8696a0" tick={{ fontSize: 11 }} />
              <Tooltip
                contentStyle={{ background: '#202c33', border: 'none', borderRadius: 8, color: '#e9edef', fontSize: 12 }}
                cursor={{ fill: 'rgba(134,150,160,0.15)' }}
              />
              <Legend wrapperStyle={{ fontSize: 11, color: '#8696a0' }} />
              <Bar dataKey="Active" fill="#00a884" radius={[4, 4, 0, 0]} />
              <Bar dataKey="Signed out" fill="#f15e6c" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        ) : (
          <div style={{ color: '#8696a0', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
