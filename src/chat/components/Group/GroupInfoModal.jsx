  ...
  useEffect(() => {
    const loadMemberDetails = async () => {
      ...
      try {
        ...
        // Resolve display records: prefer rich object, otherwise fetch from user_profiles
        const memberDetails = await Promise.all(uids.map(async (uid) => {
          ...
          try {
            ...
            if (userProfile) {
              return {
                uid,
                displayName: userProfile.display_name || userProfile.email?.split('@')[0] || 'Unknown User',
