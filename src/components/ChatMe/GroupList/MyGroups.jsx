...
import { SupabaseGroupService } from '../services/SupabaseGroupService';
...
const MyGroupsInner = ({ searchQuery: externalSearchQuery, onGroupViewChange }) => {
  // Initialize currentUser from Supabase auth
  ...
  const addGroupToSelectedFolder = async (id) => {
    if (!currentUser?.id || !selectedFolder?.id || !id) return;
    try {
      // optimistic updates
      ...
      await SupabaseGroupService.addGroupsToFolder(currentUser.id, selectedFolder.id, [id]);
    } catch (e) {
  ...
  // Current user is initialized from Supabase auth, passed via context if needed
  ...
  useEffect(() => {
    ...
    // Fetch and subscribe to user groups using service
    const unsubscribe = SupabaseGroupService.subscribeToUserGroups(currentUser.id, async (groupsArray) => {
      ...
      for (const data of groupsArray) {
        ...
        if (groupsMap.has(groupId)) {
          console.warn(`Duplicate group detected: ${groupId}`);
          continue;
        }
        
        // Use denormalized last message from group doc (updated by backend)
        let lastMessage = 'No messages yet';
        let lastMessageTimestamp = new Date(data.created_at) || new Date();
        if (data.last_message_text) {
          const senderName = data.last_message_sender_name || 'Someone';
          lastMessage = `${senderName}: ${data.last_message_text}`;
        }
        ...
        groupsMap.set(groupId, {
          ...
          unreadCount: unread,
          isMuted: mutedIds.has(groupId),
          isLocked: lockedIds.has(groupId),
          isArchived: archivedIds.has(groupId),
          isPinned: pinnedIds.has(groupId),
          avatar: {
  ...
  useEffect(() => {
    ...
    const unsubscribe = SupabaseGroupService.subscribeToUserGroupFolders(currentUser.id, (foldersList) => {
  ...
  const createFolder = async (name) => {
    ...
    try {
      const folder = await SupabaseGroupService.createGroupFolder(currentUser.id, name);
      setShowFolderCreate(false);
      setActiveTab('folders');
      if (pendingGroupIds.length > 0) {
        await SupabaseGroupService.addGroupsToFolder(currentUser.id, folder.id, pendingGroupIds);
        setFolders(prev => prev.map(f => f.id === folder.id ? { ...f, members: Array.from(new Set([...(f.members||[]), ...pendingGroupIds])) } : f));
      }
      setPendingGroupIds([]);
  ...
  const confirmAddToFolders = async (folderIds) => {
    try {
      ...
      if (!currentUser?.id || ids.length === 0 || pendingGroupIds.length === 0) return;
      let updatedAny = false;
      for (const fid of ids) {
        try {
          await SupabaseGroupService.addGroupsToFolder(currentUser.id, fid, pendingGroupIds);
          updatedAny = true;
        } catch (e) {
          console.error(`Failed to add groups to folder ${fid}:`, e);
        }
      }
      if (updatedAny) {
  ...
  const removeGroupsFromFolder = async (ids) => {
    if (!currentUser?.id || !selectedFolder || !Array.isArray(ids) || ids.length === 0) return;
    try {
      await SupabaseGroupService.removeGroupsFromFolder(currentUser.id, selectedFolder.id, ids);
  ...
  const renameCurrentFolder = async (newName) => {
    ...
    try { 
      await SupabaseGroupService.updateGroupFolder(currentUser.id, selectedFolder.id, { name: newName }); 
  ...
  const deleteCurrentFolder = async () => {
    if (!currentUser?.id || foldersToDelete.length === 0) return;
    try {
      for (const f of foldersToDelete) { 
        await SupabaseGroupService.deleteGroupFolder(currentUser.id, f.id); 
      }
  ...
  const toggleGroupFolderPin = async (folder) => {
    ...
    try {
      const newPinnedState = !folder.is_pinned;
      await SupabaseGroupService.updateGroupFolder(currentUser.id, folder.id, { is_pinned: newPinnedState });
      setFolders((prev) => prev.map(f => f.id === folder.id ? { ...f, is_pinned: newPinnedState } : f));
    } catch (e) {
      ...
      openSetPinModal(async () => {
        try {
          for (const f of foldersToLock) {
            await SupabaseGroupService.updateGroupFolder(currentUser.id, f.id, {
              is_locked: true,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // Persist leave using Supabase service
      await SupabaseGroupService.removeGroupMember(groupId, currentUser.id);
      // Immediately leave WS room
      try { leaveGroup(groupId, currentUser.id); } catch {}
  ...
  const handleUndoLeave = async () => {
    ...
    try {
      ...
      await SupabaseGroupService.addGroupMember(groupId, currentUser.id);
      // The live listener will bring it back automatically
    } catch (e) {
  ...
  const handleBulkLeave = async () => {
    ...
    try {
      // Remove from all groups using Supabase service
      await Promise.all(ids.map(id => SupabaseGroupService.removeGroupMember(id, currentUser.id)));
      // Leave WS and unsubscribe topics for each
      ids.forEach(id => { try { leaveGroup(id, currentUser.id); } catch {} });
      if (fcmToken) {
